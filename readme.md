...
`.distanceTo()` **лучше не применять**, если в сцене много объектов. Это ресурсозатаратно, т.к. в каждой операции считается извлечение корня. Дистанция до объекта находится как корень квадратный из суммы квадратов проекций по осям x и y от объекта до объекта. Т.е. по теореме Пифагора. Более **быстрая версия этой функции** - функция `.distanceToSquared()`. Она не извлекает корень при расчете дистанции.
